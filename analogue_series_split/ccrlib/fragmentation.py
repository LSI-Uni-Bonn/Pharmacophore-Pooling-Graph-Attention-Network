...
# BEGIN: Functions retrieving cuttable bonds
...
def get_fgbased_single_bonds(mol: RWMol) -> List[Tuple[Atom, Atom]]:
    """
    Determine all analog series based single bonds of a molecule. The bonds must bot be contained in a ring and
    have to be single bonds. The bonds have to be rationally analog series according to a predefined set of rules.
    :param mol: Molecule
    :return: list of bonds given as atom pairs
    """
    removable_bonds = set()
...
# END: Functions retrieving cuttable bonds

# Dictionary registering all functions for retrieving cuttable bonds

cut_type_dict = {
    "SINGLE": get_acyclic_single_bonds,
    "SYNTHESIZABLE": get_synthesizable_single_bonds,
    "RECAP": get_recap_single_bonds,
    "FG": get_fgbased_single_bonds
}
...
# TODO
# def reconstruct_molecule_with_indices(core: str, substituents: Union[str, Dict[int, str]]) -> Optional[Mol]:
#     """
#     Reassamble a molecule from kits core and fragment smiles
#     :param core: Core Smiles with substitution sites
#     :param substituents: A smiles containing fragments with the appropriate substitution sites
#     :return: reconstructed molecules as Mol object
#     """
#     if type(substituents) == dict:
#         substituents = ".".join(to_specific_rsite_map(substituents).values())
...
#         return None
#     mol = Chem.RWMol(mol)
#     core_size = Chem.MolFromSmiles(core).GetNumAtoms()
#
#     rsite_map = defaultdict(list)
#     at: Atom
#     for at in mol.GetAtoms():

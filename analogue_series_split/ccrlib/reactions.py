class IsSynthesizable:
    ...
    def detect_fg(self):
        ...
        # KT: self._record_reaction('rxn_name') was not used
        if self.track_reactions is True:
        ...
        # no need to record reactions
            ...
            # Exclude bonds with unnecessary hetero atoms below:
            # [B(5), Si(14), P(15), As(33), Se(34), Te(52)]
            if set([self.a1.GetAtomicNum(), self.a2.GetAtomicNum()]) & set([5, 14, 15, 33, 34, 52]):
                return False

            # consider Ring-attached single bond
            if self.a1.IsInRing() or self.a2.IsInRing():
                return True
            
            # remove terminal methyl group in chain structure
            if self.exclude_terminal_methyl():
                return False
            ...
            if self.a1.GetAtomicNum() == self.a2.GetAtomicNum():
                ...
                if self.a1.GetAtomicNum() == 6:  # C-C single bond
                    carbon_carbon = [self.SP_or_SP2_FG(), self.multiple_oxidized_FG()]
                    if any(carbon_carbon):
                        return True
                    ...
                    return False  # any other atom except for C

            
            # heteroatomic bonds
            else:
                if self.a1.GetAtomicNum() == 6 or self.a2.GetAtomicNum() == 6: 
                    # C-[heteroatom] single bond
                    carbon_hetero_recap = [self.amide(thio=False), self.amide(thio=True),
                                          self.ester(thio=False), self.ester(thio=True)]
                    carbon_hetero_additional = [self.single_oxidized_FG()]  ### Avoid C-F bond in C-CF2
                    if any(carbon_hetero_recap+carbon_hetero_additional):
                        return True
                    return False
                
                else: 
                    #[heteroatom]-[heteroatom] single bond 
                    hetero_hetero = [self.sulfonamide(), self.sulfonimidamide()] # only consider sulfonamide
                    if any(hetero_hetero):
                        return True
                    return False
            return False

        
    ## KT added function START ##
    def exclude_terminal_methyl(self):
        # Remove terminal methyl group because it is universal and redundant
        if (len(self._get_a1_nbrs()) == 0 and self.a1.GetAtomicNum() == 6)\
            or (len(self._get_a2_nbrs()) == 0 and self.a2.GetAtomicNum() == 6):
            return True
        return False

    def SP_or_SP2_FG(self):
        # Among carbon-carbon single bonds, single bonds such as C-C=O, C-C=C, C-C#C, C-C#N are chosen
        hybrids = ["SP2", "SP"]
        if str(self.a1.GetHybridization()) in hybrids or str(self.a2.GetHybridization()) in hybrids:
            return True
        return False

    def multiple_oxidized_FG(self):
        # Among carbon-carbon single bonds, single bonds such as C-C(OC)OC, C-C(X)X, C-C(X)(X)X are chosen
        num_oxidized_a1 = len([at for at in self._get_a1_nbrs() if at.GetAtomicNum() != 6])
        num_oxidized_a2 = len([at for at in self._get_a2_nbrs() if at.GetAtomicNum() != 6])
        #if num_oxidized_a1 >= 2 or num_oxidized_a2 >= 2:
        if num_oxidized_a1 >= 2 and num_oxidized_a1 == len(self._get_a1_nbrs()):# or num_oxidized_a2 >= 2:
            return True
        return False

    def single_oxidized_FG(self):
        # Among carbon-hetero_atom single bonds, only mono-oxidized functional groups are chosen.
        # C-O in CC(-OC)-OC, C-X in CC(-X)-X and C-X in CC(-X)(-X)-X are NOT chosen
        if self.a1.GetAtomicNum() == 6 and self.a2.GetAtomicNum() != 6:
            num_oxidized_a1 = len([at for at in self._get_a1_nbrs() if at.GetAtomicNum() != 6])
            if num_oxidized_a1 == 0:
                return True
            return False
        return False

    def sulfonimidamide(self):
        # Sulfonaimidamide  S-N single bond in -S(=O)(=NH)-N(R1)R2
        if self.a1.GetAtomicNum() == 16 and self.a2.GetAtomicNum() == 7:
            ...
            for a in self.a1.GetNeighbors():
                if a.GetAtomicNum() == 8 and self.mol_1.GetBondBetweenAtoms(a.GetIdx(),
                    ...
                    oxygen_count += 1
                elif a.GetAtomicNum() == 7 and self.mol_1.GetBondBetweenAtoms(a.GetIdx(),
                                                                            self.a1.GetIdx()).GetBondTypeAsDouble() == 2.0:
                    nitrogen_count += 1
            if oxygen_count == 1 and nitrogen_count == 1:
                return True
        return False
    ## KT added function END ##

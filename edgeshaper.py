...
class Edgeshaper():
    ...
    def __init__(self, model, x, edge_index, edge_weight = None, device = "cpu"):
        ...
        #
        self.prediction_prob_random_graphs = None
        self.fidelity = None
    ...
    def explain(self, M = 100, target_class = 0, P = None, deviation = None, log_odds = False, seed = None):
        ...
        for j in tqdm(range(num_edges)):
            ...
            for i in range(M):
                ...
                E_z_mask = torch.tensor(E_z_mask, dtype=torch.int, device=self.device)
                retained_indices_random = torch.LongTensor(torch.nonzero(E_z_mask).tolist()).to(self.device).squeeze()
                E_j_random = torch.index_select(E, dim = 1, index = retained_indices_random)
                edge_weight_j_random = None
                if self.edge_weight is not None:
                    edge_weight_j_random = torch.index_select(self.edge_weight, dim = 0, index = retained_indices_random)
                ...
                # batch = torch.zeros(x_reduced.shape[0], dtype=int, device=self.x.device)
                out = self.model(self.x, E_j_random, batch=batch, edge_weight=edge_weight_j_random)
                # out = self.model(x_reduced, E_j_random, batch=batch, edge_weight=edge_weight_j_random)
                ...
                if target_class is not None:
                    if not log_odds:
                        ...
                        V_j_random = out_prob[0][target_class].item()

                else:
                    #out_prob = out
                    out_prob= torch.sigmoid(out)
                    V_j_random = out_prob[0][0].item()


                preds_random_graphs.append(V_j_random)
                ...
                if target_class is not None:
                    ...
                    out_prob = torch.sigmoid(out)  # EDITED : sigmoid for binary classification logit
                    #print(out_prob)
        ...
        self.prediction_prob_random_graphs = preds_random_graphs
...
##########
    ...
    def compute_original_predicted_probability(self):
        if self.target_class is None:
            ...
            out_prob = torch.sigmoid(out_log_odds)
            #out_prob = F.softmax(out_log_odds, dim = 1)
            original_pred_prob = out_prob[0][0].item()
            self.original_pred_prob = original_pred_prob
            self.original_pred_class = int(self.original_pred_prob >= 0.5)
    ...
    def compute_pertinent_positive_set(self, verbose = False):
        ...
        if self.target_class is None:
            ...
            #edit: before important_edges_ranking in descending order (pos edges to negative edges)
            target_class = self.original_pred_class #get class we're trying to explain

            if target_class==0: #if explain inactivity then sort in ascending order 
                important_edges_ranking = np.argsort(np.array(self.phi_edges))
            else: # if explaining activity then sort in descending order 
                important_edges_ranking = np.argsort(-np.array(self.phi_edges)) 
            ...
            for i in range(1, important_edges_ranking.shape[0]+1):
                ...
                if self.edge_weight is not None:
                    reduced_edge_weight = torch.index_select(self.edge_weight, dim=0, index=torch.LongTensor(important_edges_ranking[0:i]).to(self.device))
                else:
                    # if you don’t have edge weights, default to all ones
                    reduced_edge_weight = torch.ones(reduced_edge_index.size(1), device=self.x.device)
                
                batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
                out_log_odds = self.model(self.x, reduced_edge_index, batch=batch, edge_weight=reduced_edge_weight)
                out_prob = torch.sigmoid(out_log_odds)
                
                #predicted_class = torch.argmax(out_prob[0]).item()
                predicted_class = int(out_prob >= 0.5)

                #print("out_prob", out_prob)
                #print("predicted_class", predicted_class)
                #print("original pred class", self.original_pred_class)
                
            
                
                #if (predicted_class == self.target_class):
                if predicted_class == self.original_pred_class:
                    #pred_prob = out_prob[0][self.target_class].item()
                    pred_prob= out_prob.item()
    ...
    def compute_minimal_top_k_set(self, verbose = False):
        ...
        if self.target_class is None:
            ...
            for i in range(important_edges_ranking.shape[0]):
                ...
                if self.edge_weight is not None:
                    reduced_edge_weight = torch.index_select(self.edge_weight, dim=0, index=torch.LongTensor(important_edges_ranking[i:]).to(self.device))
                else:
                    # if you don’t have edge weights, default to all ones
                    reduced_edge_weight = torch.ones(reduced_edge_index.size(1), device=self.x.device)
                ...
                #print("original class", self.original_pred_class)
    ...
    def compute_min_opposing_set(self, verbose=False):
        assert self.explained

        if self.original_pred_prob is None:
            self.compute_original_predicted_probability()

        if self.target_class is None:
            #print("MIN OPPOSING SET")
            self.model.eval()

            target_class = self.original_pred_class
            phi = np.array(self.phi_edges)

            # --- Split edges ---
            if target_class == 1:      # explaining activity
                opposing_edges   = np.where(phi < 0)[0]
                supporting_edges = np.where(phi > 0)[0]
                # sort positive edges high→low
                important_edges_ranking = supporting_edges[np.argsort(-phi[supporting_edges])]
            else:                      # explaining inactivity
                opposing_edges   = np.where(phi > 0)[0]
                supporting_edges = np.where(phi < 0)[0]
                # sort negative edges low→high (most negative first)
                important_edges_ranking = supporting_edges[np.argsort(phi[supporting_edges])]

            # Start with only opposing edges
            current_edges = list(opposing_edges)

            # Track *only* the added supporting edges
            added_edges = []

            # -------------------------------
            # Try adding supporting edges
            # -------------------------------
            for idx in important_edges_ranking:

                # record edge actually being added
                added_edges.append(idx)

                # construct temporary set
                new_edges = current_edges + added_edges

                reduced_edge_index = torch.index_select(
                    self.edge_index, 1, torch.LongTensor(new_edges).to(self.device)
                )

                if self.edge_weight is not None:
                    reduced_edge_weight = torch.index_select(
                        self.edge_weight, 0, torch.LongTensor(new_edges).to(self.device)
                    )
                else:
                    reduced_edge_weight = torch.ones(
                        reduced_edge_index.size(1),
                        device=self.x.device
                    )

                batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
                out_log_odds = self.model(
                    self.x, reduced_edge_index, batch=batch, edge_weight=reduced_edge_weight
                )
                out_prob = torch.sigmoid(out_log_odds)
                predicted_class = int(out_prob >= 0.5)

                if verbose:
                    print("Added edge:", idx)
                    print("out_prob:", out_prob.item())
                    print("predicted_class:", predicted_class)
                    print("original:", self.original_pred_class)

                # stop if prediction flips back to original
                if predicted_class == self.original_pred_class:
                    break

            # Edge case: if prediction never restored → return empty
            self.min_opposing_set = torch.LongTensor(added_edges).to(self.device)

            return self.min_opposing_set
